    ...
    ui(new Ui::MainWindow)
{
    ...
    qmlView->setAttribute(Qt::WA_OpaquePaintEvent);
    qmlView->setAttribute(Qt::WA_NoSystemBackground);
    qmlView->viewport()->setAttribute(Qt::WA_OpaquePaintEvent);
    qmlView->viewport()->setAttribute(Qt::WA_NoSystemBackground);
    ...
    if (isSmallResolution) {
        qmlView->setGeometry(0, 0, 480, 272);
        qmlView->setSource(QUrl::fromLocalFile("qml/smallwebbrowser/webbrowser.qml"));
    } else {
        qmlView->setSource(QUrl::fromLocalFile("qml/webbrowser/webbrowser.qml"));
    }

    QObject *rootObject = dynamic_cast<QObject*>(qmlView->rootObject());
    QDeclarativeProperty property(rootObject, QString("urlString"));
    property.write(initialURL.toLatin1());

    ui->mainLayout->addWidget(qmlView);
