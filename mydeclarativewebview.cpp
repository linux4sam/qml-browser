...
public:
    QDeclarativeWebViewPrivate(MyQDeclarativeWebView* qq)
      : q(qq)
      , preferredwidth(0)
      , preferredheight(0)
      , progress(1.0)
      , status(MyQDeclarativeWebView::Null)
      , pending(PendingNone)
    ...
    qreal progress;
    MyQDeclarativeWebView::Status status;
    QString statusText;
...
GraphicsWebView::GraphicsWebView(MyQDeclarativeWebView* parent)
...
/*!
    \qmlclass WebView MyQDeclarativeWebView
...
/*!
    \internal
    \class MyQDeclarativeWebView
    \brief The MyQDeclarativeWebView class allows you to add web content to a QDeclarativeView.
    ...
    A MyQDeclarativeWebView object can be instantiated in Qml using the tag \l WebView.
*/

MyQDeclarativeWebView::MyQDeclarativeWebView(QDeclarativeItem *parent) : QDeclarativeItem(parent)
{
    init();
}

MyQDeclarativeWebView::~MyQDeclarativeWebView()
{
    delete d;
}

void MyQDeclarativeWebView::init()
{
    ...
    d->view->setFocus();
    QDeclarativeWebPage* wp = new QDeclarativeWebPage(this);
    wp->userAgentForUrl(QUrl("http://www.epicurious.com"));
    setPage(wp);    
...
void MyQDeclarativeWebView::componentComplete()
{
...
MyQDeclarativeWebView::Status MyQDeclarativeWebView::status() const
{
    return d->status;
}
...
*/
qreal MyQDeclarativeWebView::progress() const
{
    return d->progress;
}

void MyQDeclarativeWebView::doLoadStarted()
{
    if (!d->url.isEmpty()) {
        d->status = Loading;
...
void MyQDeclarativeWebView::doLoadProgress(int p)
{
...
void MyQDeclarativeWebView::pageUrlChanged()
{
    updateContentsSize();
...
void MyQDeclarativeWebView::doLoadFinished(bool ok)
{
...
*/
QUrl MyQDeclarativeWebView::url() const
{
    return d->url;
}

void MyQDeclarativeWebView::setUrl(const QUrl& url)
{
    if (url == d->url)
        return;
...
*/
int MyQDeclarativeWebView::preferredWidth() const
{
    return d->preferredwidth;
}

void MyQDeclarativeWebView::setPreferredWidth(int width)
{
...
*/
int MyQDeclarativeWebView::preferredHeight() const
{
    return d->preferredheight;
}

void MyQDeclarativeWebView::setPreferredHeight(int height)
{
...
*/
QVariant MyQDeclarativeWebView::evaluateJavaScript(const QString& scriptSource)
{
    return this->page()->mainFrame()->evaluateJavaScript(scriptSource);
}

void MyQDeclarativeWebView::updateDeclarativeWebViewSize()
{
...
void MyQDeclarativeWebView::initialLayout()
{
    // nothing useful to do at this point
}

void MyQDeclarativeWebView::updateContentsSize()
{
    if (page()) {
...
void MyQDeclarativeWebView::geometryChanged(const QRectF& newGeometry, const QRectF& oldGeometry)
{
...
*/
QDeclarativeListProperty<QObject> MyQDeclarativeWebView::javaScriptWindowObjects()
{
    return QDeclarativeListProperty<QObject>(this, d, &QDeclarativeWebViewPrivate::windowObjectsAppend);
}

QDeclarativeWebViewAttached* MyQDeclarativeWebView::qmlAttachedProperties(QObject* o)
{
...
void QDeclarativeWebViewPrivate::updateWindowObjects()
{
    ...
    for (int i = 0; i < windowObjects.count(); ++i) {
        ...
        QDeclarativeWebViewAttached* attached = static_cast<QDeclarativeWebViewAttached *>(qmlAttachedPropertiesObject<MyQDeclarativeWebView>(object));
...
bool MyQDeclarativeWebView::renderingEnabled() const
{
    return d->rendering;
}

void MyQDeclarativeWebView::setRenderingEnabled(bool enabled)
{
...
*/
bool MyQDeclarativeWebView::heuristicZoom(int clickX, int clickY, qreal maxZoom)
{
...
/*!
    ...
    the Web engine.
*/
int MyQDeclarativeWebView::pressGrabTime() const
{
    return d->view->pressTime;
}

void MyQDeclarativeWebView::setPressGrabTime(int millis)
{
...
*/
QAction* MyQDeclarativeWebView::backAction() const
{
...
*/
QAction* MyQDeclarativeWebView::forwardAction() const
{
...
*/
QAction* MyQDeclarativeWebView::reloadAction() const
{
...
*/
QAction* MyQDeclarativeWebView::stopAction() const
{
...
*/
QString MyQDeclarativeWebView::title() const
{
...
*/
QPixmap MyQDeclarativeWebView::icon() const
{
...
*/
void MyQDeclarativeWebView::setStatusText(const QString& text)
{
    d->statusText = text;
    emit statusTextChanged();
}

void MyQDeclarativeWebView::windowObjectCleared()
{
    d->updateWindowObjects();
}

QString MyQDeclarativeWebView::statusText() const
{
    return d->statusText;
}

QWebPage* MyQDeclarativeWebView::page() const
{
    return d->view->page();
}
...
/*!
    ...
    \endqml
*/
QDeclarativeWebSettings* MyQDeclarativeWebView::settingsObject() const
{
    d->settings.s = page()->settings();
    return &d->settings;
}

void MyQDeclarativeWebView::setPage(QWebPage* page)
{
    if (d->view->page() == page)
        return;
...
void MyQDeclarativeWebView::load(const QNetworkRequest& request, QNetworkAccessManager::Operation operation, const QByteArray& body)
{
    page()->mainFrame()->load(request, operation, body);
}

QString MyQDeclarativeWebView::html() const
{
...
/*!
    ...
    \endqml
*/
void MyQDeclarativeWebView::setHtml(const QString& html, const QUrl& baseUrl)
{
    updateContentsSize();
    if (isComponentComplete())
...
void MyQDeclarativeWebView::setContent(const QByteArray& data, const QString& mimeType, const QUrl& baseUrl)
{
    updateContentsSize();
...
QWebHistory* MyQDeclarativeWebView::history() const
{
    return page()->history();
}

QWebSettings* MyQDeclarativeWebView::settings() const
{
    return page()->settings();
}

MyQDeclarativeWebView* MyQDeclarativeWebView::createWindow(QWebPage::WebWindowType type)
{
    switch (type) {
    case QWebPage::WebBrowserWindow: {
        if (!d->newWindowComponent && d->newWindowParent)
            ...
            MyQDeclarativeWebView* webview = 0;
            ...
            if (newObject) {
                ...
                if (!item)
                    ...
                    webview = item->findChild<MyQDeclarativeWebView*>();
                    if (!webview)
                        delete item;
                    else {
...
*/
QDeclarativeComponent* MyQDeclarativeWebView::newWindowComponent() const
{
    return d->newWindowComponent;
}

void MyQDeclarativeWebView::setNewWindowComponent(QDeclarativeComponent* newWindow)
{
...
*/
QDeclarativeItem* MyQDeclarativeWebView::newWindowParent() const
{
    return d->newWindowParent;
}

void MyQDeclarativeWebView::setNewWindowParent(QDeclarativeItem* parent)
{
...
QSize MyQDeclarativeWebView::contentsSize() const
{
    return page()->mainFrame()->contentsSize() * contentsScale();
}

qreal MyQDeclarativeWebView::contentsScale() const
{
    return d->view->scale();
}

void MyQDeclarativeWebView::setContentsScale(qreal scale)
{
...
QColor MyQDeclarativeWebView::backgroundColor() const
{
    return d->view->palette().base().color();
}

void MyQDeclarativeWebView::setBackgroundColor(const QColor& color)
{
...
*/
QRect MyQDeclarativeWebView::elementAreaAt(int x, int y, int maxWidth, int maxHeight) const
{
...
/*!
    ...
    \sa MyQDeclarativeWebView
*/
QDeclarativeWebPage::QDeclarativeWebPage(MyQDeclarativeWebView* parent) :
    QWebPage(parent)
{
}
...
MyQDeclarativeWebView* QDeclarativeWebPage::viewItem()
{
    return static_cast<MyQDeclarativeWebView*>(parent());
}

QWebPage* QDeclarativeWebPage::createWindow(WebWindowType type)
{
    MyQDeclarativeWebView* newView = viewItem()->createWindow(type);
    if (newView)
        return newView->page();
    return 0;
}
